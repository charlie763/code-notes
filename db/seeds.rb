...
# topics
topics_attributes = (1..10).map do |num|
  {
    name: "Topic#{num}"
  }
end
...
# languages
languages = %w[Ruby Python JavaScript C Swift]
...
notes_attributes = (1..50).each do |num|
  note = Note.create(
    ...
    user_id: User.select(:id).map(&:id).sample,
    language_id: Language.select(:id).map(&:id).sample
  )
...
ExternalResource.create(name: 'Duck Duck go', url: 'duckduckgo.com', user: User.first).notes << Note.find(3)
# resource subsection
